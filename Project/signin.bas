...
Sub Process_Globals
	...
	Private signup="SIGNUP" As String
End Sub
...
Sub Globals
	...
	Private txtoccupation As EditText
...
Sub btnupload_Click
ImageView2.Bitmap=LoadBitmap(File.DirAssets,"defaultimage.png")
End Sub
...
Sub btnsignup_Click
	...
	Else If occupation.Trim.Length==0 Then
	ToastMessageShow("Please enter your Occupation",True)			
	Else If medhistory.Trim.Length==0 Then
	ToastMessageShow("Please enter your Health Condition Status",True)
	Else
	radiomaritalstatus
	genderfill
	Dim query="INSERT INTO `user` (`User_id`, `FirstName`, `MiddleName`, `LastName`, `Age`, `maritalstatus`, `Phone`, `Email`, `Username`, `Password`, `Gender`, `Residence`, `weight`, `height`, `occupation`, `medicalhistory`, `status`, `Image`, `Updated_at`) VALUES (NULL, '"&fname&"', '"&mname&"', '"&lname&"', '"&age&"', '"&maritalstatus&"', '"&phone&"', '"&email&"', '"&username&"', '"&password&"', '"&mygender&"', '"&Residence&"', '"&weight&"', '"&height&"', '"&occupation&"', '"&medhistory&"', 'online', 'na', '"&global.currentDateTime&"')" As String	
	ExecuteRemoteQuery(query, signup)
	ToastMessageShow("Successfully added "&username&" as User,Now log in as user",True)		
	End If		
