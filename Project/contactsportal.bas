...
Sub Process_Globals
	...
	Private contactsspec="CSPEC" As String	
End Sub
...
Sub Activity_Create(FirstTime As Boolean)
	...
	'Activity.LoadLayout("Layout1")
     Activity.LoadLayout("contacts")
	 ProgressDialogShow("Loading specialists, please wait...")
	 fillscrollview
End Sub
...
Sub fillscrollview
  Dim query="SELECT * FROM specialist WHERE Specialization='"&myspec&"'" As String	
...
Sub JobDone(Job As HttpJob)
	...
	If Job.Success Then	   
		...
		parser.Initialize(res)		
			If Job.JobName==contactsspec Then		
				...
				Dim myuserid As String=rootMap.Get("Spec_id")
				...
				Dim task As String=rootMap.Get("Specialization")
			...
			userimage.Initialize(File.DirAssets,"defaultimagespec.png")
			PanelTop=0
			...
			'buttons
			myspecid=m.Get("Spec_id")
			btn.Initialize("btn")
			btn.Tag=myspecid
			subpanel.AddView(btn,250dip,20dip,70dip,40dip)
			btn.Text="Chat!"			
	...
	ToastMessageShow("No Specialists Found", True)
	End Try
	Job.Release
End Sub
